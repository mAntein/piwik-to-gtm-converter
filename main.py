...
async def convert_piwik_gtm(file: UploadFile = File(...)):
    ...
    # Use only valid GTM parameter types
    ...
    for trigger_index, (trigger_id, trigger) in enumerate(piwik_json.get('triggers', {}).items(), start=1):
        ...
        gtm_event_type = "page_view"  # Default to page_view if unknown
    ...
    for tag_index, (tag_id, tag) in enumerate(piwik_json.get('tags', {}).items(), start=1):
        ...
        parameters = []
        for param in tag.get("attributes", {}).get("parameters", []):
            param_type = parameter_type_mapping.get(param.get("type", "TEMPLATE"), "TEMPLATE")
            parameters.append({
                "type": param_type,
                "key": param.get("key", ""),
